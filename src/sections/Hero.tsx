...
export function Hero() {
  ...
  const activeParticles = mobileLite ? STAR_PARTICLES.filter((_, index) => index % 2 === 0) : STAR_PARTICLES
  const activeDots = mobileLite ? STAR_DOTS.filter((_, index) => index % 2 === 0) : STAR_DOTS
  const activeBursts = mobileLite ? STAR_BURSTS.slice(0, 4) : STAR_BURSTS
    ...
    () => {
      ...
      if (!mobileLite) {
        ...
        particles.forEach((particle, index) => {
          const config = activeParticles[index]
        ...
        bursts.forEach((burst, index) => {
          const config = activeBursts[index]
          gsap.to(burst, {
            scale: 1.14,
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        ...
        <div className="absolute inset-x-[8%] top-[9%] h-40 rounded-full bg-[radial-gradient(circle,rgba(255,228,158,0.18)_0%,rgba(255,228,158,0.06)_28%,transparent_72%)] blur-2xl sm:h-56 sm:blur-3xl md:inset-x-[20%]" />
        <div className="absolute -left-12 top-[22%] h-32 w-32 rounded-full bg-[#c89bbf]/8 blur-[72px] sm:h-48 sm:w-48 sm:blur-[110px] md:h-72 md:w-72" />
        <div className="absolute -right-10 top-[14%] h-36 w-36 rounded-full bg-[#e8983c]/10 blur-[72px] sm:h-56 sm:w-56 sm:blur-[120px] md:h-80 md:w-80" />
        ...
        <div className="absolute inset-0 hidden opacity-[0.26] mix-blend-overlay lux-noise md:block" />
      </div>

      <div className="pointer-events-none absolute inset-0" aria-hidden>
        {activeParticles.map((star, i) => (
          <span
            key={`particle-${i}`}
      ...
      <div className="relative z-30 mx-auto flex min-h-[100dvh] max-w-7xl flex-col px-4 pb-10 pt-4 sm:px-5 sm:pb-12 sm:pt-5 md:px-10 md:pb-16 md:pt-7">
        <div className="flex items-center justify-between text-[9px] uppercase tracking-[0.36em] text-[#f7dfa0]/60 sm:text-[10px] sm:tracking-[0.45em] md:text-[11px]">
          <div className="flex items-center gap-3">
            <div
              ref={crescentRef}
              className="flex h-10 w-10 items-center justify-center rounded-full border border-gold/15 bg-[#160c24]/78 p-1.5 shadow-[0_0_24px_rgba(255,214,120,0.1)] sm:h-12 sm:w-12 md:h-14 md:w-14 md:p-2 md:backdrop-blur-md"
            >
              <img
                src={CRESCENT_URL}
                alt=""
        ...
        <div className="relative flex-1">
          <div
            ref={sceneRef}
            className={`pointer-events-none absolute inset-x-0 top-0 min-h-[26.5rem] sm:min-h-[31rem] md:inset-0 md:min-h-[36rem] ${mobileLite ? '' : 'will-change-transform'}`}
          >
            ...
            <div
              ...
              style={{
                ...
                filter: mobileLite ? 'blur(16px)' : 'blur(22px)',
              }}
              aria-hidden
            />
